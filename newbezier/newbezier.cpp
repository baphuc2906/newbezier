...
void DrawBezierCurve(int n, BezierCurve bcurve)
{
	int i;
	ofstream output;
	output.open("D:/do an/code doancoso/newbezier/newbezier/output.txt", std::fstream::out| std::ofstream::app);
...
int check() {
	...
	int numchars = 0;
	int num = 0;
	file.open("D:/do an/code doancoso/newbezier/newbezier/input.txt");
	//file >> x;
	file.get(c);
	while (file) {
		while (file && c != '\n') {
...
void display(Point2Struct  d[]) {
	cout << "Tap diem thuoc duong cong benzier: ";
	for (int i = 0; i < numlines; i++) {
		d[i].show();
	}
	cout << "\n";
	...
	output << "\n              -Tang Ba Hong Phuc";
	output << "\n Lop : 18TCLC_DT3";
	output << "\n De tai cua nhom là: Noi suy duong cong Bezier";
...
int main()
{
    ...
    Point2Struct  d[MAXPOINTS];/*    {	  Digitized points  : tập hợp điểm*/
	int choose = 0; 
	...
	cin >> choose;
		...
		case 1:
			...
			input(d);
			display(d);
			FitCurve(d, ::numlines, error);		/*  Fit the Bezier curves :khớp với đường cong bezier */
			break;
		case 2:
			int n;
